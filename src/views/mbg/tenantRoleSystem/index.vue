  ...
  <div class="app-container">
    <el-card class="filter-container" shadow="never">
        <div>
          ...
          <el-button
            style="float: right"
            @click="searchTenantRoleSystemList()"
            type="primary"
            size="small">
            查询结果
          </el-button>
        ...
        <div style="margin-top: 15px">
          <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
            ...
            <el-form-item label="开放：">
              <el-select v-model="listQuery.roleSysOn" placeholder="全部" clearable>
                <el-option
                  v-for="item in roleSysOnOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <el-button
        class="btn-add"
        @click="addTenantRoleSystem()"
        size="mini">
        添加
      </el-button>
    </el-card>
    <div class="table-container">
      <el-table ref="tenantRoleSystemTable"
                :data="list"
                style="width: 100%"
        ...
        <el-table-column label="开放" width="100" align="left" header-align="center">
          <template slot-scope="scope">{{scope.row.roleSysOn | formatRoleSysOn}}</template>
        </el-table-column>
...
<script>
  import {fetchList, deleteTenantRoleSystem} from '@/api/tenantRoleSystem'
  ...
  export default {
    ...
    data() {
      return {
        ...
        total: null,
        listLoading: true,
        roleSysOnOptions: Object.assign({},defaultRoleSysOnOptions),
        systemDesignOptions:[],
        tenantInfoOptions:[],
        multipleSelection: []
      }
    ...
    filters:{
      formatRoleSysOn(roleSysOn){
        for(let i=0;i<defaultRoleSysOnOptions.length;i++){
          if(roleSysOn===defaultRoleSysOnOptions[i].value){
            return defaultRoleSysOnOptions[i].label;
          }
        }
      },
    },
    methods: {
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/mbg/updateTenantRoleSystem', query: {id: row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('是否要删除该角色模块', '提示', {
          ...
          deleteTenantRoleSystem(row.id).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 1000
      ...
      addTenantRoleSystem() {
        this.$router.push({path: '/mbg/addTenantRoleSystem'})
      }
    }
  }
</script>
