...
<script>
  import {fetchList, deleteTenantModule, updateModuleStatus} from '@/api/tenantModule'
  ...
  export default {
    ...
    methods: {
      ...
      handleView(index, row) {
        this.$router.push({path: '/module/viewTenantModule', query: {id: row.id, tenantId:this.tenantId}})
      },
      handleUpdate(index, row) {
        this.$router.push({path: '/module/updateTenantModule', query: {id: row.id, tenantId:this.tenantId}})
      },
      ...
      handleModuleStatusChange(index, row) {
        let data = new URLSearchParams();
        ;
        data.append("ids", row.id);
        data.append("moduleStatus", row.moduleStatus);
        updateModuleStatus(data).then(response => {
          this.$message({
            message: '修改成功',
            type: 'success',
            duration: 1000
      ...
      addTenantModule() {
        this.$router.push({path: '/module/addTenantModule', query: {tenantId:this.tenantId}})
      }
    }
  }
</script>
