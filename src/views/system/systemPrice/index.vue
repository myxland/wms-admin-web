<template> 
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <el-button
        class="btn-add"
        @click="addSystemPrice()"
        size="mini">
        添加
      </el-button>
    </el-card>
    <div class="table-container">
      <el-table ref="systemPriceTable"
                :data="list"
                style="width: 100%"
...
<script>
  import {fetchList, deleteSystemPrice} from '@/api/systemPrice'
  ...
  export default {
    ...
    methods: {
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/system/updateSystemPrice', query: {id: row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('是否要删除该模块价格', '提示', {
          ...
          deleteSystemPrice(row.id).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
              duration: 1000
      ...
      addSystemPrice() {
        this.$router.push({path: '/system/addSystemPrice'})
      }
    }
  }
</script>
